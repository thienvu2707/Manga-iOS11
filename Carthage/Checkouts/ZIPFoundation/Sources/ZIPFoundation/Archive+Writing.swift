...
public extension Archive {
    fileprivate enum ModifyOperation: Int {
        case remove = -1
        case add = 1
    }
    ...
    public func addEntry(with path: String, type: Entry.EntryType, uncompressedSize: UInt32,
        ...
        var localFileHeader = try self.writeLocalFileHeader(path, compressionMethod: compressionMethod,
                                                            size: (uncompressedSize, 0), checksum: 0,
                                                            modificationDateTime: modDateTime)
        let (sizeWritten, checksum)  = try self.writeEntry(localFileHeader, type: type,
        ...
        localFileHeader = try self.writeLocalFileHeader(path, compressionMethod: compressionMethod,
        ...
        let centralDirectory = try self.writeCentralDirectoryStructure(localFileHeader,
                                                                       relativeOffset: offset,
                                                                       externalFileAttributes: externalFileAttributes)
        if UInt32(startOfCentralDirectory) > UINT32_MAX {
            throw ArchiveError.invalidStartOfCentralDirectoryOffset
        }
        let start = UInt32(startOfCentralDirectory)
        endOfCentralDirectoryRecord = try self.writeEndOfCentralDirectory(centralDirectory,
    ...
    public func remove(_ entry: Entry, bufferSize: UInt32 = defaultReadChunkSize) throws {
        ...
        let endOfCentralDirectoryRecord = try
            tempArchive.writeEndOfCentralDirectory(entry.centralDirectoryStructure,
    ...
    fileprivate func writeLocalFileHeader(_ path: String, compressionMethod: CompressionMethod,
    ...
    fileprivate func writeEntry(_ localFileHeader: LocalFileHeader, type: Entry.EntryType,
        ...
        case .file:
            ...
            case .none:
                (sizeWritten, checksum) = try self.writeUncompressed(localFileHeader.uncompressedSize,
                                                                     bufferSize: bufferSize, provider: provider)
            case .deflate:
                (sizeWritten, checksum) = try self.writeCompressed(localFileHeader.uncompressedSize,
                                                                   bufferSize: bufferSize, provider: provider)
            }
        case .directory: _ = try provider(0, 0)
        case .symlink:
            (sizeWritten, checksum) = try self.writeSymbolicLink(localFileHeader.uncompressedSize,
                                                                 provider: provider)
        }
        return (sizeWritten, checksum)
    }

    fileprivate func writeUncompressed(_ size: UInt32, bufferSize: UInt32,
    ...
    fileprivate func writeCompressed(_ size: UInt32, bufferSize: UInt32,
                                 provider: Provider) throws -> (sizeWritten: UInt32, checksum: CRC32) {
        var sizeWritten = 0
    ...
    fileprivate func writeSymbolicLink(_ size: UInt32, provider: Provider) throws -> (sizeWritten: UInt32, checksum: CRC32) {
    ...
    fileprivate func writeCentralDirectoryStructure(_ localFileHeader: LocalFileHeader, relativeOffset: UInt32,
    ...
    fileprivate func writeEndOfCentralDirectory(_ centralDirectoryStructure: CentralDirectoryStructure,
    ...
    fileprivate func replaceCurrentArchiveWithArchive(at URL: URL) throws {
